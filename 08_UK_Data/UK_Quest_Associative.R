...
ggplot(quest_clean_UK, aes(v_169, fill = v_331 == 1 )) + 
  geom_bar(position = "fill")+ scale_fill_brewer( palette = "Paired") +
  coord_flip() +
  scale_y_continuous(labels = scales::percent, minor_breaks = seq(1,25,25)) + 
  theme_apa(remove.x.gridlines = F) +
...
# Blockchain: v_321
# 1 = yes, 2 = no
heard_app <- quest_clean_UK[, .(v_321, v_265, v_266, v_267, v_268, v_269, v_270)]
...
block_usage <- quest_clean_UK[,c("v_28", "v_334")]
...
gender <- quest_clean_UK[, .("Gender" = v_169)]
...
# Knowledge 
# (1-10 scale): v_286
knowledge <- quest_clean_UK[, .(v_286)]
...
# Calculate Average 
quest_clean_UK[, Perceived_risk := round(rowMeans(quest_clean_UK[, .(v_149, v_150)], na.rm = T), 2)]
perceived_risk <- quest_clean_UK[, .(Perceived_risk)]
...
# Calculate Average 
quest_clean_UK[, Usage_Intention := round(rowMeans(quest_clean_UK[, .(v_132, v_133)], na.rm = T), 2)]
usage_int <- quest_clean_UK[, .(Usage_Intention)]
